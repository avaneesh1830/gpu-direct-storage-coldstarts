...
MODELS = {
    ...
    # 405B AWQ (~203 GB) — genuinely requires multi-GPU; use with --tp-size 8
    "405b": ("hugging-quants/Meta-Llama-3.1-405B-Instruct-AWQ-INT4", "awq_marlin", "float16"),
}
...
def bench_load(model_id, quant, dtype, enforce_eager, load_format, prompt_len, gen_tokens, tp_size):
...
def main():
    ...
    parser.add_argument("--tp-size", type=int, default=1,
                        help="tensor_parallel_size — set to GPU count for multi-GPU (e.g. 8) nodes")
    ...
    if args.mode == "disk":
        ...
        metrics = bench_load(model_id, quant, dtype, args.enforce_eager,
                             args.load_format, args.prompt_len, args.gen_tokens, args.tp_size)
